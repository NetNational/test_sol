...
contract RentBasic {
	...
	address public punishAddr = 0x17B0477C515A5615f18F0E70DF144D4ec13607cf; 
	...
	constructor(address _token, address _register, address _remarkAddr) {
		owner = msg.sender;
	...
	*/
	...
	function commentHouse(bytes32 _houseId, uint8 _ratingIndex, string _ramark) public returns(bool) {
		...
		require(reInfo.existed, "Not find house!");
	 	...
	 	address _refeAddr;
	 	if (houseInfos[_houseId].landlord == sender) {  // 房东
	 		_refeAddr = l2rMaps[sender];
	 	} else {
	 		_refeAddr = houseInfos[_houseId].landlord;
	 	}
	 	bool flag = remarkContract.commentHouse(_houseId, houseInfos[_houseId].landlord, _refeAddr, _ratingIndex, _ramark);
	 	if (!flag) {
	 		return false;
	 	}
