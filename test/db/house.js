...
function updateReleaseInfo(conn, state, userId, addr) {
	...
	return new Promise((resolve, reject) => {
		conn.then(con => {
			con.query("UPDATE `house_release_info` SET `house_state` = ?, `updatetime` = ? WHERE `addr` = ?", [state, Date.now(), addr], function(err, result, fileds){
...
// 房东签订合同记录表
function insertAgreeRecord(conn, userName, phoneNum, addr, houseAddr, rental, tenancy, txHash, houseId, falsify, houseDeadline) {
		...
		conn.then(con => {
			con.query("SELECT * FROM house_transaction_record WHERE addr = ? ", [addr],  function (err, result, fields) {
			    ...
			    if (result != null && result.length != 0) {
			    	...
			    	let state = 0;
					let insertSql = "INSERT INTO house_transaction_record (`userid`, `addr`, `house_addr`, `tenancy`, `rental`, `house_id`, `tx_hash`, `state`, `pay_deadline`, `pay_one`, `landlord_sign_time`,`createtime`, `updatetime`) VALUES ?";
					let addParam = [[phoneNum, addr, houseAddr, tenancy, rental, houseId, txHash, state, 3, "甲方", Date.now(), Date.now(), Date.now()]]; // Mul
					con.query(insertSql, [addParam], function(err, result, fileds){
						console.log("--insert release info-----",result);
						if (err) {
							console.log("release insert error:", err);
