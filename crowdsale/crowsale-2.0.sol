    ...
    **/
    ...
    function YingRentSale(
        ...
        address tokenContractAddress) {
            ...
            fundingGoal = fundingGoalInEthers * 100;
            deadline = now + durationInMinutes * 1 minutes;
            price = finneyCostOfEachToken * 1000;
    ...
    function raiseBlt(uint amount) public returns(bool) {
        require(!crowdsaleClosed);
        tokenReward.transferFrom(msg.sender, beneficiary, amount.Div(price).Mul(10**8)); // Token decimal is 8
        balanceOf[msg.sender] = balanceOf[msg.sender].Add(amount);
        amountRaised = amountRaised.Add(amount);
        // tokenReward.transfer(msg.sender, amount.Div(price).Mul(10**8)); // Token decimal is 8
        FundTransfer(msg.sender, amount, true);
