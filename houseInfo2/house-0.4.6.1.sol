...
contract RentBasic {
	...
	constructor(ERC20Interface _token, address _register) {
		owner = msg.sender;
		token = RentToken(_token);
		userRegister = UserRegister(_register);
	}
	
    modifier onlyLogin() {
		require(userRegister.isLogin(msg.sender), "User must login!");
		_;
	}
	/**
	 * title lease
	...
	*/
	 ...
	 */
	 function withdraw(bytes32 _houseId, uint amount) public onlyLogin returns(bool) {
...
// 	}
	 ...
	 */
	function commentHouse(bytes32 _houseId, uint8 _ratingIndex, string _ramark) public onlyLogin returns(bool) {
		address sender = msg.sender;
		HouseReleaseInfo reInfo = hsReleaseInfos[_houseId];
		require(reInfo.existed, "Not find the house");
