...
interface RegisterInterface {
	function isRegister() public constant returns(bool);
...
contract Authentication {
	...
	function authHouse(uint _guid) public returns(bytes32) {
		require(!alAuthlist[_guid], "This house already be Authentication!");
		address sender = msg.sender;
		uint _idCard;
		string _owername;
		(_idCard, _owername) = userRegister.getPersonInfo(sender);
